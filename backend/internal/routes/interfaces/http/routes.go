...
func RegisterRoutes(mux *http.ServeMux, handler *Handler, tokens authdomain.TokenService) {
	...
	mux.Handle("GET /api/v1/routes", protected)
	mux.Handle("POST /api/v1/routes", protected)
	mux.Handle("GET /api/v1/routes/{id}", protected)
	mux.Handle("PATCH /api/v1/routes/{id}", protected)
	mux.Handle("DELETE /api/v1/routes/{id}", protected)
}
