...
func RegisterRoutes(mux *http.ServeMux, handler *Handler, tokens authdomain.TokenService) {
	...
	mux.Handle("GET /api/v1/vehicles", protected)
	mux.Handle("POST /api/v1/vehicles", protected)
	mux.Handle("GET /api/v1/vehicles/{id}", protected)
	mux.Handle("PATCH /api/v1/vehicles/{id}", protected)
	mux.Handle("DELETE /api/v1/vehicles/{id}", protected)
}
